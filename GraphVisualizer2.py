...
class TreeVisualizer(Widget):
	...
	def add_node(self, i, input_position):
		print "ADDING NODE", i
		new_node = Node(i, tag="n-test", pos=input_position, radius=self.node_radius)
		self.node_widgets[i] = new_node
		self.graph_nodes.add_widget(new_node)
	
	def add_edge(self, i, input_tail, input_head):
		new_edge = Edge(i, tag="e-test", tail=input_tail, head=input_head)
