...
class CircularButton(Button):
	...
	def interact(self, value):
		if self.get_ctrl().touch_button == 'left':
			self.background_color = brighten(self.color, click_highlight_val)
...
class Node(FloatLayout):
	...
	def highlight(self, color_shift=None):
		...
		if not self.highlit:
			...
			lum = lum - luminescence(new_color)
			new_color = brighten(new_color, (lum + 1)*highlight_val)
			self.ids['handle'].color = new_color
			print self.i, "highlit", lum, new_color
...
class Edge(FloatLayout):
	...
	def highlight(self, color_shift=None):
		...
		if not self.highlit:
			...
			lum = luminescence(self.normal_color)
			
			#generate new color
			new_color = color_complement(self.normal_color)
			
			lum = lum - luminescence(new_color)
			
			r, g, b, a = brighten(new_color, (lum + 1)*highlight_val)
			#assign light_switch value to alpha
			new_color = r, g, b, 1
			
			self.color = new_color
			self.ids['handle'].color = self.color
			print self.i, "highlit", lum
...
class TreeVisualizer(Widget):
	...
	def add_edge(self, i, input_tail, input_head, input_data=None, input_color=None):
		new_edge = Edge(i, data=input_data, color=input_color, tail=input_tail, head=input_head)
		new_edge = Edge(i, data=input_data, color=input_color, tail=input_tail, head=input_head)
